\documentclass[letter,10pt]{article}
\usepackage[utf8]{inputenc}

%====================
% OFFICIAL PUBLIC OVERLEAF TEMPLATE
% https://www.overleaf.com/latex/templates/data-science-tech-resume-template/zcdmpfxrzjhv
%
%====================
%
% Resume template for data scientists, a complement to data-science-tech-cover-letter-template:
% https://www.overleaf.com/latex/templates/data-science-tech-cover-letter-template/gbrcqktbsfxf
%
% Files:        resume.tex: Main file
%               _header.tex: header code
%               TLCresume.sty: style file containing formatting details
%               section/objective: https://www.indeed.com/career-advice/resumes-cover-letters/resume-objective-examples
%               section/skills: table of skills
%               section/experience: projects or roles
%               section/education: schools and stuff
%               section/activities: optional, could comment out in resume.tex.
%
% Editor:       https://github.com/TimmyChan
%               https://www.linkedin.com/in/timmy-l-chan/
%
% Last Updated: March 24th, 2022
%
%====================

\usepackage[english,french]{babel}
\usepackage{TLCresume}
\usepackage{fontawesome5}
\usepackage{setspace}
\usepackage{hyperref}
\usepackage{multicol}
\usepackage{tikz}
\newcommand{\cvtag}[1]{%
  \tikz[baseline]\node[anchor=base,draw=black!30,rounded corners,inner xsep=.75ex,inner ysep =0.35ex,text height=1.5ex,text depth=.25ex]{#1};
}


%=====================
% INTERNATIONALIZATION
%=====================

\newif\ifen
\newif\iffr

\newcommand{\en}[1]{\ifen#1\fi}
\newcommand{\fr}[1]{\iffr#1\fi}

\ifdefined\english
  \entrue
\else \ifdefined\french
  \frtrue
\else % Default (or debug) language
  \entrue   % English
  % \frtrue   % French
\fi
\fi

%====================
% INFORMATION
%====================
\def\name{Renato C. Juaçaba Neto} % Name Here
\def\phone{+33 (0)6 13 18 16 93}
\def\email{renatocjn@gmail.com}
\def\LinkedIn{renatocjn} % linkedin.com/in/______
\def\github{renatocjn} % github username
\def\role{% JOB TITLE
\en{Network Researcher \& Junior Developer}
\fr{Chercheur \& Développeur junior}
}
\def\birthday{25 Jun 1992}
\def\nationality{\en{Brazilian}\fr{Brésilien}}
\def\address{Strasbourg, FR}
\def\languages{% Languages spoken
\en{English {\scriptsize (C2)}, portuguese {\scriptsize (C2)}, french {\scriptsize (B2)}}
\fr{Anglais{\scriptsize (C2)}, portugais {\scriptsize (C2)}, français {\scriptsize (B2)}}
}


\begin{document}

\input{_header}

% \begin{minipage}{.74\textwidth}
% \input{sections/objective}
% \end{minipage}

% \begin{minipage}{.24\textwidth}
%   \includegraphics[width=1in]{photo}
% \end{minipage}

\en{\section{Skills}}
\fr{\section{Compétences}}
\input{sections/skills}

\en{\section{Experience}}
\fr{\section{Expérience}}
\input{sections/experience}

\en{\section{Education}}
\fr{\section{Éducation}}
\input{sections/education}

\en{\section{Main Publications}}
\fr{\section{Principales publications}}
\input{sections/publications}

\en{\section{Other Activities}}
\fr{\section{Autres activités}}
\input{sections/activities}

\en{\section{References}}
\fr{\section{Références}}
\input{sections/references}


\end{document}
